/* UK-Intergalactic scenario goal (active for that run — comment/uncomment as needed):
attackGoal(lateralMovementVPN('intergalactic-hacker', 'intergalactic-vpn', 'alpine-3.18-openvpn-1')).
*/
...
dataBind(Flow, 'intergalactic-vpn-gw-internet', Path1).
...
isInSubnet('outsideNet','intergalactic-hacker-internet').
isInSubnet('outsideNet','intergalactic-hacker').
isInSubnet('vpn','intergalactic-hacker').
isInSubnet('outsideNet','uk-site-internet').
isInSubnet('outsideNet','MainRouter').
isInSubnet('insideNet','MainRouter').
isInSubnet('insideNet','Core-Switch').
isInSubnet('hpNet','HP-2650D').
isInSubnet('hpNet','HP-2650D1').
isInSubnet('hpNet','HP-2560A').
isInSubnet('voiceNet','VoIP-Switch').
isInSubnet('wifiCtrlNet','AP-Controller').
isInSubnet('wifiNet','DEV3').
isInSubnet('wifiNet','FLE4N').
isInSubnet('labNet','alpine-3.18-openvpn-1').
...
networkService('storage-server-1', _, 'sftpd', '22', 'user-backup-account').
networkService('intergalactic-vpn', _, 'open-ssh', '22', 'root').
networkService('intergalactic-hacker', _, 'open-ssh', '22', 'hacker').
...
networkService('MainRouter', 'fwMgmtUI', 'tcp', '443', _).
networkService('Core-Switch', _, 'open-ssh', '22', _).
networkService('HP-2560A', _, 'open-ssh', '22', _).
networkService('HP-2650D1', _, 'open-ssh', '22', 'hpAdmin').
networkService('HP-2650D', _, 'open-ssh', '22', _).
networkService('VoIP-Switch', _, 'open-ssh', '443', _).
networkService('AP-Controller', _, 'open-ssh', '80', 'apcAdmin').
networkService('AP-Controller', _, 'open-ssh', '443', 'apcAdmin').
networkService('ADMIN', _, 'open-ssh', '22', 'localAdmin').
networkService('PAXTON', _, 'open-ssh', '22', 'paxUser').
...
hasAccount('alice-backup', 'storage-server-1', 'user-backup-account').
hasAccount('hacker', 'intergalactic-hacker', 'user-account').
hasAccess('hacker', 'intergalactic-hacker', 'intergalactic-vpn', 'udp', '1194').
hasAccess('hacker', 'intergalactic-hacker', 'intergalactic-vpn', 'http', '80').
hasAccess('alice', 'alpine-3.18-openvpn-1', 'intergalactic-vpn', 'udp', '1194').
located('intergalactic-vpn-gw-internet', 'outsideNet', 'External/WAN').
...
fileOwner('alpine-3.18-openvpn-1', './etc/shr', 'alice-backup').
fileOwner('AP-Controller', Path, 'root').
fileOwner('ADMIN', Path, 'root').
fileOwner('PAXTON', Path, 'root').
fileOwner('storage-server-1', Path, 'root').
...
ownerAccessible('AP-Controller', 'read', Path).
ownerAccessible('ADMIN', 'read', Path).
ownerAccessible('PAXTON', 'read', Path).
ownerAccessible('storage-server-1', 'read', Path).
allows('intergalactic-vpn', 'hacker', _, _, '200').
allows('intergalactic-vpn', 'alice', _, _, '200').
setUIDProgram('intergalactic-vpn', 'intergalactic-web-ui', 'Account').
installed('intergalactic-vpn', 'intergalactic-web-ui', '2.4.12').
canInvoke('intergalactic-web-ui', 'Software2', 'keylogging').
accessFile('root', 'intergalactic-vpn', 'write', Path).
isNameResolver('NameResolver', 'intergalactic-vpn', 'intergalactic-hacker').
isNameResolver('NameResolver', 'intergalactic-hacker', 'intergalactic-vpn').
vulE2EProtocol('intergalactic-vpn', 'NameResolver', 'dnsCachePoisoning', 'dns', DNSProt, 'remoteExploit', 'nameresolverCachePoisoned').
localService('intergalactic-vpn', 'intergalactic-vpn', 'Account').
...
residesOn('intergalactic-hacker', 'intergalactic-web-ui', '2.4.12').
residesOn('intergalactic-vpn', 'open-ssh', 'v1.0').
residesOn('alpine-3.18-openvpn-1', 'intergalactic-web-ui', '0.1.0rc0').
residesOn('alpine-3.18-openvpn-1', 'intergalactic-web-ui', '2.4.12').
residesOn('alpine-3.18-openvpn-1', 'open-ssh', 'v1.0').
residesOn('storage-server-1', 'sftpd', 'v1.0').
vulExists('cve-2023-27524', 'intergalactic-web-ui', '2.4.12', 'network', 'ca_loss', 'critical').
...
/****   Modeled after the validated Caldera run    *****/
/****   (Thief-UK-OFFICE-test-3-file-20260707)     *****/
/******************************************************/

/* --- Attacker foothold ---
   Thief assumes the attacker already has a foothold on ADMIN
   (representing the manually installed / already-established
   Sandcat agent used in the validated Caldera run). Unlike
   UK-Intergalactic, no exploit chain is modeled to reach this
   foothold — agentPresent/2 is the seed fact, and execCode/3 is
   derived from it via a bridging rule in IRs_UK-Office_Thief.p,
   keeping execCode consistently `derived` across both scenarios
   (it is NOT redeclared as primitive here).

   NOTE: malicious(attacker) and malicious('attacker') are the same
   ground fact in Prolog — already declared above, not repeated. */

agentPresent('ADMIN', root).
hasAccount(attacker, 'ADMIN', root).

/* --- Sensitive files present on ADMIN (matches planted test files) --- */

fileHasExtension('ADMIN', '/root/secrets.yml', yml).
fileHasExtension('ADMIN', '/root/photo.png', png).
fileHasExtension('ADMIN', '/root/recording.wav', wav).
...
/* --- C2 / Caldera server reachability ---
   Represents the confirmed NAT-fixed path: ADMIN (192.168.1.0/24)
   -> MainRouter -> cyber-twin0 -> gns3-server (MASQUERADE)
   -> caldera (172.18.0.6:8888). 'caldera' is intentionally NOT a
   GNS3 topology node (it is an external Docker service reached the
   same way from any node) — the topology reduction script correctly
   drops it as an invalid AG node when building the reduced cyber twin. */

c2Server('caldera').
hacl('ADMIN', 'Core-Switch', tcp, '8888').
hacl('Core-Switch', 'MainRouter', tcp, '8888').
hacl('MainRouter', 'uk-site-internet', tcp, '8888').
hacl('uk-site-internet', 'caldera', tcp, '8888').

/* --- Active attack goal for this run: Thief --- */

attackGoal(thiefCampaign(attacker, 'ADMIN', 'caldera')).
