...
interaction_rule(
   (netAccess(User, AttackSrc, Host, Protocol, Port) :-
     execCode(User, AttackSrc, _Account),
     hacl(AttackSrc, Host, Protocol, Port),
     malicious(User)),
   rule_desc('Compromised host reaches the next hop via a firewall/ACL-permitted egress path', 1.0)).

interaction_rule(
   (netAccess(User, AttackSrc, Host2, Protocol, Port) :-
     netAccess(User, AttackSrc, Host1, Protocol, Port),
     hacl(Host1, Host2, Protocol, Port)),
   rule_desc('ACL-permitted network hop', 1.0)).

/******************************************************/
/****   Thief scenario Interaction Rules (appended) *****/
/****   Data theft / exfiltration, post-compromise  *****/
/******************************************************/

/* --- New predicate declarations (Thief-specific only) ---
   malicious/1, hasAccount/3, hasAccess/5, and execCode/3 are
   already declared above (in the UK-Intergalactic base IR) and
   are reused as-is — NOT redeclared here.

   IMPORTANT FIX vs. the original standalone Thief IR draft:
   execCode/3 was previously declared `primitive` here, which
   conflicts with its `derived` declaration above (derived from
   compromised(Host) + vulExists(...) in the UK-Intergalactic
   chain). That primitive declaration has been REMOVED. Thief's
   foothold is now expressed via a new primitive, agentPresent/2,
   with a bridging rule below that derives execCode from it —
...
/* Bridging rule: derive execCode from an already-present agent,
   instead of the full RCE chain UK-Intergalactic models. This is
   the fix for the primitive/derived conflict noted above. */
interaction_rule(
   (execCode(User, Host, Account) :-
     agentPresent(Host, Account),
     malicious(User)),
   rule_desc('Attacker already has an established foothold - pre-installed Caldera agent', 1.0)).

/* Interaction Rule for T1005 - Data from Local System */
interaction_rule(
   (fileFound(User, Host, Path) :-
     execCode(User, Host, _Account),
     fileHasExtension(Host, Path, Ext),
     sensitiveFileExtension(Ext),
     malicious(User)),
   rule_desc('Locate files with a sensitive extension on a compromised host', 1.0)).

/* Interaction Rule for T1074.001 - Local Data Staging */
interaction_rule(
...
/* Interaction Rule for T1560.001 - Archive via Utility */
interaction_rule(
   (dataArchived(User, Host, 'staged.tar.gz') :-
     fileStaged(User, Host, _Path, _StagingDir)),
...
interaction_rule(
   (dataExfiltrated(User, Host, ArchiveFile, C2) :-
     ...
     netAccess(User, Host, C2, tcp, '8888'),      % was: hasAccess(User, Host, C2, tcp, '8888')
     c2Server(C2)),
